...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, 
    ...
    style_img = image_loader("flower4.png")
    # style_img = image_loader("spiral.png")
    ...
    stylizedDeformGrid = StyliziedPureVoxelGO(
            xyz_min=xyz_min_overwrite, xyz_max=xyz_max_overwrite,
            deform_num_voxels= 90**3,
            deform_num_voxels_base=90**3,
            H =render_resoultion,W = render_resoultion,
            focal=focal,c2w= c2w,far=far,near=near
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        for i in tqdm(range(FrameCount)):
            ...
            if UseNNFM:           
                def GetStyliziedImageRGBLoss(rays_o, rays_d, time_one, viewdirs,cam_one,Model,FrameNum,content_weight = 100, NNFMContent = NNFMContent, **render_kwargs):
                    ...
                    loss_dict["content_loss"] *= 1e-3 * NNFMContent
                    loss_dict["img_tv_loss"] = img_tv_loss * 0.000001
                    
                    # style_weight = 1000000
                    loss_ccp = 0.0
                    loss = sum(list(loss_dict.values())) * style_weight
                    ...
                    print('current content_score = ' + str(loss_dict["content_loss"].item() * 1000000  ))
                    print('current style_score = ' + str(loss_dict["nnfm_loss"].item() * 1000000  ))
                    if enableTVLoss:
            ...
            if Enable_allignTrain:     
                ...
                for substeps in range(align_substeps_count):
                    ...
                    if UsingBeforeImageTraining:
                        ...
                        RGBGradTotal, rgbResult = GetStyliziedImageRGBLoss(rays_o, rays_d, time_one, viewdirs,cam_one, TotalModel,
                                                                            # prev_VGGmodel,prev_style_losses,
                    ...
                    for ro, rd, vd ,ts,cams,grad_before in zip(rays_o.split(bacth_size, 0), rays_d.split(bacth_size, 0),
                                ...
                                #with torch.no_grad():
                        results = TotalModel.forward(ro, rd, vd,ts, cams, Enablek0Bypass=True,FrameNum= FrameNum,DeltaTime_T=deltaT, **render_kwargs)                  
            ...
            if j == targetFrameIndex:
                ...
                optimizer = utils.create_optimizer_for_styliziedDefromationGrid_transportBased_withAdditionalRadField(TotalModel, cfg_train, global_step=0,lrscale= lrscale,lrate_decay=cfg_train.lrate_decay,
                                                                                                ...
                                                                                                opt_radNet = True,opt_OriNeRFModelColors = optOriNeRFColorField
                                                                                                )
            else:
                # lrscale = 0.001
