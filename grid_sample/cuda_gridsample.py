...
gridsample_grad2 = load(name='gridsample_grad2', sources=['grid_sample/gridsample_cuda.cpp', 'grid_sample/gridsample_cuda.cu'], verbose=True)
...
class _GridSample3dForward(torch.autograd.Function):
    ...
    @staticmethod
    def forward(input, grid, padding_mode=0, align_corners=True):
        # assert input.ndim == 5
        # assert grid.ndim == 5
        # assert input.shape[0] == grid.shape[0]
    ...
    def setup_context(ctx, inputs, output):
        input, grid, padding_mode, align_corners = inputs
        ctx.save_for_backward(input, grid)
        ctx.padding_mode = ['zeros', 'border'].index(padding_mode)
        ctx.align_corners = align_corners 
    @staticmethod
...
class _GridSample3dBackward(torch.autograd.Function):
    generate_vmap_rule = True
    # @staticmethod
    # def forward(grad_output, input, grid, padding_mode=0, align_corners=True):
    #     op,info = torch._C._jit_get_operation('aten::grid_sampler_3d_backward')
    #     # print(op)
    #     # if _use_pytorch_1_11_api:
    #     #     output_mask = (ctx.needs_input_grad[1], ctx.needs_input_grad[2])
    #     #     grad_input, grad_grid = op(grad_output, input, grid, 0, padding_mode, align_corners, output_mask)
    ...
    #     # output_mask = (ctx.needs_input_grad[1], ctx.needs_input_grad[2])
    #     # grad_input, grad_grid = op(grad_output, input, grid, 0, padding_mode, align_corners, output_mask)
    #     grad_input, grad_grid = op(grad_output, input, grid,  0, padding_mode, align_corners,[True,True])
    ...
    @staticmethod
    def forward( grad_output, input, grid, padding_mode='border', align_corners=True):
        # ctx.save_for_backward(grid)
        grad_input = torch.zeros_like(input)
        grad_grid = torch.zeros_like(grid)
        # print('may be here should not be reached')
        return grad_input, grad_grid
    ...
    def setup_context(ctx, inputs, output):
        # print('ssssssss')
        # print(len(inputs))
        grad_output, input, grid, padding_mode, align_corners = inputs
        ctx.save_for_backward(input, grid)
        ctx.padding_mode = ['zeros', 'border'].index('border')
        ctx.align_corners = align_corners 
