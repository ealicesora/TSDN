...
train_config = dict(
    ...
    lrate_densityGrid=1e-2,
    lrate_deform_k0= 0.01 , 
    lrate_sty_k0=1e-2,                # lr of color/feature voxel grid
    lrate_sty_deformnet=1e-4 ,         # lr of the mlp to estimate deformation
    Sty_lrate_rgbnet = 2e-3 * 0.1,
    Sty_lrate_k0 = 5e-2 * 0.1,
    lrate_k0=8e-2,                # lr of color/feature voxel grid
    lrate_deformnet=8e-4,         # lr of the mlp to estimate deformation
    
    Sty_lrate_Ori_Model_rgbnet = 2e-2 ,
    # Sty_lrate_Ori_Model_rgbnet = 1e-2 * 0.5, # for cows
    Sty_lrate_Ori_Model_feature_copy = 2e-2,
    Sty_lrate_Ori_Model_featurenet = 2e-2,
    
    
    deform_weight_tv_k0 = 1e10,
    ...
    weight_tv_feature= 1e-2,
    pg_scale=[2000, 4000, 6000, 8000],
    skip_zero_grad_fields=['feature'],
    elastics_loss_weight = -0.1,
    DeltaTime = 1.0/50.0,
    BFNCC_Loss_weight = -1e-2,
...
model_and_render = dict(
    ...
    deform_num_voxels=190**3,           # expected number of voxel
    deform_num_voxels_base=190**3,      # to rescale delta distance
