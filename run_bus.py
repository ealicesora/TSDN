...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, 
    ...
    # prepare reference
        for i in tqdm(range(FrameCount)):
            ...
            contentImages_ori.append(noGradRGBTarget.clone())
        ...
        for i in tqdm(range(FrameCount)):
            ...
            if UseNNFM:           
                def GetStyliziedImageRGBLoss(rays_o, rays_d, time_one, viewdirs,cam_one,Model,FrameNum,content_weight = 100, NNFMContent = NNFMContent, style_weight=style_weight,**render_kwargs):
                    ...
                    curImage = contentImages_ori[current_Frame].cuda()      
                    ...
                    refImg = contentImages_ori[current_Frame].to('cuda')
                    # print(refImg.shape)
                    # print(noGradRGBTarget.shape)
                    # return
                    if USECCPL:
            ...
            rgbResult_origin = None
            if j != targetFrameIndex:
                OPSyn_Target_Reference = synthesizeImage(Stylizied_Results[current_Save_Frame-1],contentImages_ori[current_Save_Frame-1],contentImages_ori[current_Save_Frame],current_Save_Frame)
