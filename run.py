...
def config_parser():
    ...
    parser.add_argument("--fre_test", type=int, default=3000,
...
def compute_bbox_by_cam_frustrm_hyper(args, cfg,data_class):
    ...
    print('compute_bbox_by_cam_frustrm: xyz_max', xyz_max)
    print('compute_bbox_by_cam_frustrm: finish')
    return xyz_min, xyz_max, None




def compute_bbox_by_cam_frustrm_hyper_new(args, cfg,data_class):
    print('compute_bbox_by_cam_frustrm: start')
    xyz_min = torch.Tensor([np.inf, np.inf, np.inf])
    xyz_max = -xyz_min
    BBX_TIme = []
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict):
    ...
    if cfg.data.dataset_type !='hyper_dataset':
        ...
        deltaTime = 1.0 / len(i_train)
        frameCount = len(i_train)
        print(deltaTime)
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        calcuateEalstsicsloss = False
        if cfg_train.elastics_loss_weight > 0.0 and global_step> 0:
            calcuateEalstsicsloss = True
        # volume rendering
        render_result = model(rays_o, rays_d, viewdirs, times_sel, global_step=global_step,calcuateEalstsicsloss=calcuateEalstsicsloss, 
                              DeltaTime = deltaTime,
                              **render_kwargs)
        ...
        RenderLoss = loss.item()/cfg_train.weight_main
        ...
        if cfg_train.elastics_loss_weight > 0.0 and global_step> 3000:
            elastic_loss =  render_result['elastics_loss']
            elastic_loss = elastic_loss.sum(axis=-1).mean()
            loss += cfg_train.elastics_loss_weight * elastic_loss
        ...
        if cfg_train.delta_norm_l1>0 :
            if model.deformationVeoxl_net != None:
                # if 
                # deltas = torch.abs(render_result['deltas']) 
                deltas = torch.abs(render_result['delta_delta']) 
                deltas = torch.mean(deltas) 
                loss += deltas * cfg_train.delta_norm_l1
                #print(deltas.shape)
                
                
        
        if cfg_train.deform_weight_tv_k0>0 :
            if model.deformationVeoxl_net != None:
                dtv = model.deformationVeoxl_net.k0_total_variation()
                res = cfg_train.deform_weight_tv_k0 * dtv
                # if global_step > 2000:
                #     print(res.item())
                loss += res
        ...
        if global_step > 2000 and cfg_train.BFNCC_Loss_weight > 0.0:
            frame = global_step % frameCount 
            if frame != 0:
                times =  frame * deltaTime
                BFNCC_Loss += model.BFECCReg(times, deltaTime)
                
                BFNCC_Loss.backward()     

        Norm_Loss = 0.0
        
        if global_step > 2000 and cfg_train.Norm_Loss_weight > 0.0:
            frame = global_step % frameCount 
            if frame != 0:
                times =  frame * deltaTime
                Norm_Loss += model.NormLoss(times, deltaTime)
        ...
        if global_step%args.i_print == 0:
            ...
            tqdm.write(f'scene_rep_reconstruction : iter {global_step:6d} / '
                       f'TotalLoss: {loss:.9f} /'
                       f'RenderLoss: {RenderLoss:.9f} / PSNR: {np.mean(psnr_lst):5.2f} / '
                       f'Eps: {eps_time_str}')
            psnr_lst = []
        ...
        if global_step%(args.fre_test) == 0:
            ...
            if cfg.data.dataset_type != 'hyper_dataset': 
                ...
                rgbs,disps = render_viewpoints_hyper(
                    ...
                    savedir=testsavedir, all=True, test=True, eval_psnr=args.eval_psnr,
                    **render_viewpoints_kwargs)
...
def train(args, cfg, data_dict=None):
    ...
    if cfg.data.dataset_type == 'hyper_dataset':
        xyz_min, xyz_max,_ = compute_bbox_by_cam_frustrm_hyper(args = args, cfg = cfg,data_class = data_dict['data_class'])
    else:
        xyz_min, xyz_max,_ = compute_bbox_by_cam_frustrm(args = args, cfg = cfg, **data_dict)
