...
def argmin_cos_distance(a, b, center=False):
    ...
    for i in range(0, a.shape[-1], loop_batch_size):
        ...
        ind1 = torch.topk(d_mat,KNNCount,2).indices

        z_best_batch = ind1
        z_best.append(z_best_batch)
    z_best = torch.cat(z_best, dim=-2)
...
def nn_feat_replace(a, b):
    ...
    b_ref = b_flat.clone().unsqueeze(-1)
    # print(a.size())
    # print(b.size())
    # exit()
    z_new = []
    for i in range(n):
        ...
        z_best = z_best.unsqueeze(1).repeat(1, c, 1,1)
        
        b_ref = b_ref.repeat(1, 1, 1,KNNCount)
    ...
    z_new = z_new.view(n, c, h, w,KNNCount)
    return z_new


def cos_loss(a, b):
    #print(a.shape)
    a = a.unsqueeze(-1).repeat(1,1,1,1,KNNCount)
    # print(a.shape)
    # print(b.shape)
    # # exit(-1)
